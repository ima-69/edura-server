...
router.get("/dashboard", auth(true, ['student']), asyncHandler(
...
router.get("/courses", auth(true, ['student']), asyncHandler(
  async (req: AuthRequest, res: Response) => {
    const studentId = req.user?.id;
...
// Enroll in a course
router.post("/courses/enroll/:courseId", auth(true, ['student']), asyncHandler(
  async (req: AuthRequest, res: Response) => {
    const studentId = req.user?.id;
    ...
    if (student.classes?.includes(courseId as any)) {
    ...
    student.classes.push(courseId as any);
    await student.save();
...
// Get student's exams
router.get("/exams", auth(true, ['student']), asyncHandler(
  async (req: AuthRequest, res: Response) => {
    const studentId = req.user?.id;
...
// Get student profile
router.get("/profile", auth(true, ['student']), asyncHandler(
  async (req: AuthRequest, res: Response) => {
    const studentId = req.user?.id;
...
router.put("/profile", auth(true, ['student']), asyncHandler(
  async (req: AuthRequest, res: Response) => {
    const studentId = req.user?.id;
