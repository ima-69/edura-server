...
export const updateRegister = async (req, res) => {
    try {
        ...
        const result = await Register.findOneAndUpdate({ student_id: id }, registerSafeParse.data, { new: true });
        if (!result) {
            return res.status(404).json({
...
export const registerClassByFindClassId = async (req, res) => {
    try {
        const id = req.query.class_id;
        if (!id || !mongoose.Types.ObjectId.isValid(id)) {
            return res.status(400).json({ message: "Invalid class id" });
        }
        const result = await Register.find({ class_id: id }).sort({ createdAt: -1 });
...
export const registerClassByFindStudentId = async (req, res) => {
    try {
        const id = req.query.student_id;
        if (!id || !mongoose.Types.ObjectId.isValid(id)) {
            return res.status(400).json({ message: "Invalid student id" });
        }
        const result = await Register.find({ student_id: id }).sort({ createdAt: -1 });
        const resultArr = result.map((register) => ({
            register_id: register._id,
            class_id: register.class_id,
            student_id: register.student_id,
...
export const deleteRegisterClassByRegisterId = async (req, res) => {
    try {
        ...
        const result = await Register.findOneAndUpdate({ student_id: studentId }, // search by student_id
