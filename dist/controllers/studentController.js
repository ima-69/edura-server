...
export const updateStudent = async (req, res) => {
    try {
        ...
        const student = await Student.findByIdAndUpdate(id, parsed.data, { new: true });
        if (!student) {
            return res.status(404).json({
...
export const changeStudentStatus = async (req, res) => {
    try {
        ...
        const student = await Student.findByIdAndUpdate(id, [{ $set: { student_status: { $not: "$student_status" } } }], { new: true } // return updated doc
        );
        if (!student) {
            return res.status(404).json({
