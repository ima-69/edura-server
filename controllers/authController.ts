...
const studentToken = (Student: { _id: { toString: () => string }, email?: string, first_name: string, last_name: string, mobile?: string, student_status: boolean }): string => {
...
const adminToken = (Admin: { _id: { toString: () => string }, email: string, name: string }): string => {
    const secret = process.env.JWT_SECRET;
    if (!secret) {
        throw new Error('JWT_SECRET is not defined');
    }
